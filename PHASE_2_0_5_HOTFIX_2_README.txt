...
HOTFIX 2 NOTE
The invitation modal used the generic DOM id "inviteMessage", which is also used by the invitation onboarding screen. getElementById could therefore target the hidden onboarding message instead of the modal. HOTFIX 2 isolates the modal message as "userInviteMessage" and bumps the frontend cache version to GUVEL-2-0-5-HF2. No database changes.
