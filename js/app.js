...
async function createInvitation(){const email=document.getElementById('inviteEmail').value.trim().toLowerCase(),role=document.getElementById('inviteRole').value,msg=document.getElementById('inviteMessage'),btn=document.getElementById('createInvite');if(!email){msg.className='status error';msg.textContent='Email is required.';return;}btn.disabled=true;msg.className='status';msg.textContent='Creating invitation...';try{const {data,error}=await sb.rpc('create_company_invitation',{target_company_id:activeCompanyId,target_email:email,target_role:role});if(error)throw error;const token=typeof data==='string'?data:data?.token||data?.invitation_token||'';msg.className='status success';msg.innerHTML=`<strong>Invitation created.</strong><div class="invite-token-box"><span>${escapeHtml(token||'Token generated by server')}</span><button class="secondary" id="copyInviteToken" type="button">Copy Token</button></div><small>Share this token securely with the invited user. It is returned only once by the invitation function.</small>`;document.getElementById('copyInviteToken')?.addEventListener('click',async()=>{const b=document.getElementById('copyInviteToken');try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(token);}else{const ta=document.createElement('textarea');ta.value=token;ta.setAttribute('readonly','');ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();const ok=document.execCommand('copy');ta.remove();if(!ok)throw new Error('Clipboard copy failed');}b.textContent='Copied';setTimeout(()=>{b.textContent='Copy Token';},1600);}catch(_){b.textContent='Copy failed';setTimeout(()=>{b.textContent='Copy Token';},1800);}});setTimeout(()=>switchUsersTab('invitations'),900);}catch(e){msg.className='status error';msg.textContent=e.message||'Unable to create invitation.';}finally{btn.disabled=false;}}
