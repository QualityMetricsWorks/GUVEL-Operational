...
let dashboardState={tab:'General',production:[],scrap:[],downtime:[],customers:[],parts:[],shifts:[],machines:[],filters:{from:'',to:'',customer:'',part:'',shift:'',machine:''}};
function dashboard(){const tabs=['General','Production','Quality','Performance'];return head('Dashboard','Connected operational visibility.')+`<div class="tabs dashboard-tabs">${tabs.map((x,i)=>`<button class="tab ${i===0?'active':''}" data-dashboard-tab="${x}">${x}</button>`).join('')}</div><div id="dashboardFilters" class="panel dashboard-filter-panel"><div class="section-title"><div><h2>Global Filters</h2><p>One filter scope drives all Dashboard indicators.</p></div><div class="actions" style="margin-top:0"><button id="dashboardClear" class="secondary" type="button">Clear Filters</button><button id="dashboardRefresh" class="secondary" type="button">Refresh</button></div></div><div class="form-grid dashboard-filters"><div class="field"><label>Date From</label><input id="dashFrom" type="date"></div><div class="field"><label>Date To</label><input id="dashTo" type="date"></div><div class="field"><label>Customer</label><select id="dashCustomer"><option value="">All Customers</option></select></div><div class="field"><label>Part Number</label><select id="dashPart"><option value="">All Part Numbers</option></select></div><div class="field"><label>Shift</label><select id="dashShift"><option value="">All Shifts</option></select></div><div class="field"><label>Machine</label><select id="dashMachine"><option value="">All Machines</option></select></div></div><div id="dashboardStatus" class="status"></div></div><div id="dash"></div>`}
function dashTab(t){
  dashboardState.tab=t;
  const dash=document.getElementById('dash'); if(!dash)return;
  if(t!=='General'){dash.innerHTML=`<div class="panel dashboard-coming"><div class="eyebrow">PHASE 1.9</div><h2>${escapeHtml(t)} Dashboard</h2><p>This section is reserved for Phase 1.9.${t==='Production'?'C':'D'}. The global Dashboard filter contract is already shared.</p><span class="dashboard-phase-badge">Implementation deferred</span></div>`;return;}
  dash.innerHTML=`<div id="dashboardGeneral"></div>`;renderDashboardGeneral();
}
...
function dashboardFiltered(){const f=dashboardState.filters;const prod=dashboardState.production.filter(r=>(!f.from||r.production_date>=f.from)&&(!f.to||r.production_date<=f.to)&&(!f.customer||r.customer_id===f.customer)&&(!f.part||r.part_number_id===f.part)&&(!f.shift||r.shift_id===f.shift)&&(!f.machine||r.machine_id===f.machine));const ids=new Set(prod.map(r=>r.id));return {prod,scrap:dashboardState.scrap.filter(r=>ids.has(r.production_capture_id)),downtime:dashboardState.downtime.filter(r=>ids.has(r.production_capture_id))};}
function dashboardAggregate(){const d=dashboardFiltered();const scrapByCapture=new Map(),downByCapture=new Map();for(const r of d.scrap)scrapByCapture.set(r.production_capture_id,(scrapByCapture.get(r.production_capture_id)||0)+Number(r.quantity||0));for(const r of d.downtime){const a=downByCapture.get(r.production_capture_id)||{planned:0,unplanned:0,unknown:0,total:0};const m=Number(r.minutes||0);a.total+=m;const type=String(r.event_type||'').toLowerCase();if(type==='planned')a.planned+=m;else if(type==='unplanned')a.unplanned+=m;else a.unknown+=m;downByCapture.set(r.production_capture_id,a);}
 let production=0,scrap=0,scrapCost=0,planned=0,operating=0,perfNumerator=0,validOee=0,invalidOee=0,unknownEvents=0,plannedDowntime=0,unplannedDowntime=0;const daily=new Map();
 for(const r of d.prod){const q=Number(r.production_quantity||0);const sq=scrapByCapture.get(r.id)||0;production+=q;scrap+=sq;scrapCost+=sq*Number(r.part_numbers?.scrap_cost||0);const dt=downByCapture.get(r.id)||{planned:0,unplanned:0,unknown:0,total:0};plannedDowntime+=dt.planned;unplannedDowntime+=dt.unplanned;unknownEvents+=dt.unknown>0?1:0;const sh=r.shifts;const shiftSec=shiftDurationSeconds(sh);const excluded=Number(sh?.excluded_planned_minutes||0)*60;const plannedSec=shiftSec==null?null:Math.max(0,shiftSec-excluded*1);const cycle=Number(r.operations?.ideal_cycle_time_seconds||0);if(plannedSec!=null&&cycle>0&&dt.unknown===0){const opSec=Math.max(0,plannedSec-dt.unplanned*60);planned+=plannedSec;operating+=opSec;perfNumerator+=cycle*q;validOee++;}else invalidOee++;const key=r.production_date||'No Date';const day=daily.get(key)||{production:0,scrap:0};day.production+=q;day.scrap+=sq;daily.set(key,day);}
 const good=production-scrap;const yieldRatio=production>0?good/production:null;const scrapPct=production>0?scrap/production:null;const ppm=production>0?scrap/production*1e6:null;const availability=planned>0?operating/planned:null;const performance=operating>0?perfNumerator/operating:null;const quality=production>0?good/production:null;const oee=availability!=null&&performance!=null&&quality!=null?availability*performance*quality:null;return {d,production,scrap,scrapCost,good,yieldRatio,scrapPct,ppm,planned,operating,perfNumerator,availability,performance,quality,oee,validOee,invalidOee,unknownEvents,plannedDowntime,unplannedDowntime,daily};}
function renderDashboardGeneral(){const box=document.getElementById('dashboardGeneral');if(!box)return;const a=dashboardAggregate();if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p></div>';dashboardStatus('No Data in the selected filter scope.');return;}const warning=a.unknownEvents||a.invalidOee?`<div class="notice dashboard-data-note">Data quality: ${a.unknownEvents?`${a.unknownEvents} capture(s) contain unknown downtime classification. `:''}${a.invalidOee?`${a.invalidOee} capture(s) were excluded from OEE calculations because operation, ideal cycle time, shift timing or downtime classification was not resolvable.`:''}</div>`:'';const trend=[...a.daily.entries()].sort((x,y)=>x[0].localeCompare(y[0])).slice(-14);const maxProd=Math.max(...trend.map(x=>x[1].production),1);const maxScrap=Math.max(...trend.map(x=>x[1].scrap),1);box.innerHTML=`${warning}<div class="grid dashboard-kpis"><div class="card"><div class="label">OEE</div><div class="metric">${dashPct(a.oee)}</div><div class="label">Valid captures: ${a.validOee.toLocaleString()}</div></div><div class="card"><div class="label">Production</div><div class="metric">${dashNum(a.production)}</div><div class="label">Total pieces</div></div><div class="card"><div class="label">Scrap</div><div class="metric">${dashNum(a.scrap)}</div><div class="label">${dashPct(a.scrapPct)} of production</div></div><div class="card"><div class="label">PPMs</div><div class="metric">${a.ppm==null?'N/A':Math.round(a.ppm).toLocaleString()}</div><div class="label">Piece-based</div></div><div class="card"><div class="label">Yield</div><div class="metric">${dashPct(a.yieldRatio)}</div><div class="label">Good / production</div></div><div class="card"><div class="label">Direct Scrap Cost</div><div class="metric">${dashMoney(a.scrapCost)}</div><div class="label">COPQ Proxy</div></div></div><div class="grid dashboard-secondary-kpis"><div class="card"><div class="label">Availability</div><div class="metric">${dashPct(a.availability)}</div></div><div class="card"><div class="label">Performance</div><div class="metric">${dashPct(a.performance)}</div></div><div class="card"><div class="label">Planned Downtime</div><div class="metric">${dashNum(a.plannedDowntime)} min</div></div><div class="card"><div class="label">Unplanned Downtime</div><div class="metric">${dashNum(a.unplannedDowntime)} min</div></div></div><div class="section"><div class="panel"><div class="section-title"><div><h2>Production & Scrap Trend</h2><p>Last ${trend.length} production dates in the selected scope.</p></div></div><div class="dashboard-trend">${trend.map(([day,v])=>`<div class="trend-row"><div class="trend-label">${escapeHtml(day)}</div><div class="trend-bars"><div class="trend-line"><span class="trend-name">Production</span><div class="bar-track"><div class="bar production" style="width:${Math.max(2,(v.production/maxProd)*100)}%"></div></div><strong>${dashNum(v.production)}</strong></div><div class="trend-line"><span class="trend-name">Scrap</span><div class="bar-track"><div class="bar scrap" style="width:${Math.max(v.scrap?2:0,(v.scrap/maxScrap)*100)}%"></div></div><strong>${dashNum(v.scrap)}</strong></div></div></div>`).join('')}</div></div></div><div class="grid dashboard-summary-panels"><div class="panel"><h2>OEE Calculation</h2><p>Availability ${dashPct(a.availability)} × Performance ${dashPct(a.performance)} × Quality ${dashPct(a.quality)} = <strong>${dashPct(a.oee)}</strong></p></div><div class="panel"><h2>Quality Summary</h2><p>Good Quantity: <strong>${dashNum(a.good)}</strong> · Scrap Cost: <strong>${dashMoney(a.scrapCost)}</strong></p></div></div>`;dashboardStatus(`${a.d.prod.length.toLocaleString()} production captures · ${a.scrap.length.toLocaleString()} scrap events · ${a.downtime.length.toLocaleString()} downtime events.`);}
