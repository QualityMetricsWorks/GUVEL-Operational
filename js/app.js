...
function closeDashboardConfig(){document.getElementById('dashboardConfigModal')?.classList.add('hidden');}
function saveKpiSettings(key){const read=id=>{const v=document.getElementById(id)?.value??'';return kpiInputToStored(key,v);};const p=dashboardPrefs();p.kpis[key]={greater:{enabled:document.getElementById('kpiGreaterEnabled').checked,value:read('kpiGreaterValue'),color:document.getElementById('kpiGreaterColor').value},less:{enabled:document.getElementById('kpiLessEnabled').checked,value:read('kpiLessValue'),color:document.getElementById('kpiLessColor').value},between:{enabled:document.getElementById('kpiBetweenEnabled').checked,min:read('kpiBetweenMin'),max:read('kpiBetweenMax'),color:document.getElementById('kpiBetweenColor').value}};saveDashboardPrefs(p);closeDashboardConfig();renderDashboardGeneral();}
function saveChartSettings(key){const pct=DASH_CHART_META[key].unit==='%',read=id=>{const v=document.getElementById(id)?.value??'';if(v==='')return null;const n=Number(v);return Number.isFinite(n)?(pct?n/100:n):null;};const p=dashboardPrefs();p.charts[key]={min:read('chartMin'),max:read('chartMax'),target:read('chartTarget')};saveDashboardPrefs(p);closeDashboardConfig();renderDashboardGeneral();}
function resetDashboardConfig(kind,key){const p=dashboardPrefs();if(kind==='kpi')delete p.kpis[key];else delete p.charts[key];saveDashboardPrefs(p);closeDashboardConfig();renderDashboardGeneral();}
function bindDashboardConfig(){document.querySelectorAll('[data-kpi-settings]').forEach(b=>b.onclick=()=>openKpiSettings(b.dataset.kpiSettings));document.querySelectorAll('[data-chart-settings]').forEach(b=>b.onclick=()=>openChartSettings(b.dataset.chartSettings));const e=document.getElementById('dashboardConfigModal');if(!e)return;e.onclick=ev=>{if(ev.target===e||ev.target.closest('[data-config-close]'))return closeDashboardConfig();const ks=ev.target.closest('[data-kpi-save]');if(ks)return saveKpiSettings(ks.dataset.kpiSave);const cs=ev.target.closest('[data-chart-save]');if(cs)return saveChartSettings(cs.dataset.chartSave);const rs=ev.target.closest('[data-config-reset]');if(rs){const title=e.querySelector('h2')?.textContent||'';const key=Object.keys(DASH_KPI_META).find(k=>DASH_KPI_META[k].label===title)||Object.keys(DASH_CHART_META).find(k=>DASH_CHART_META[k].label===title);return resetDashboardConfig(DASH_KPI_META[key]?'kpi':'chart',key);}};}
...
function renderDashboardGeneral(){const box=document.getElementById('dashboardGeneral');if(!box)return;const cmp=dashboardComparison(),a=cmp.current,prev=cmp.previous,p=cmp.period;if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p></div>';dashboardStatus('No Data in the selected filter scope.');return;}
 ...
 <div class="card kpi-card kpi-oee"><div class="kpi-top"><div class="label">OEE</div>${kpiGear('oee')}</div><div class="metric kpi-value-oee">${dashPct(a.oee)}</div>${kpiDeltaMarkup(a.oee,prev.oee,'points',true,priorLabel)}</div>
 <div class="card kpi-card kpi-production"><div class="kpi-top"><div class="label">Production</div>${kpiGear('production')}</div><div class="metric kpi-value-production">${dashNum(a.production)}</div>${kpiDeltaMarkup(a.production,prev.production,'relative',true,priorLabel)}</div>
 <div class="card kpi-card kpi-scrap"><div class="kpi-top"><div class="label">Scrap</div>${kpiGear('scrap')}</div><div class="metric kpi-value-scrap">${dashPct(a.scrapPct)}</div>${kpiDeltaMarkup(a.scrapPct,prev.scrapPct,'points',false,priorLabel)}</div>
 <div class="card kpi-card kpi-ppm"><div class="kpi-top"><div class="label">PPMs</div>${kpiGear('ppm')}</div><div class="metric kpi-value-ppm">${a.ppm==null?'N/A':Math.round(a.ppm).toLocaleString()}</div>${kpiDeltaMarkup(a.ppm,prev.ppm,'relative',false,priorLabel)}</div>
 <div class="card kpi-card kpi-yield"><div class="kpi-top"><div class="label">Yield</div>${kpiGear('yield')}</div><div class="metric kpi-value-yield">${dashPct(a.yieldRatio)}</div>${kpiDeltaMarkup(a.yieldRatio,prev.yieldRatio,'points',true,priorLabel)}</div>
 <div class="card kpi-card kpi-copq"><div class="kpi-top"><div class="label">COPQ</div>${kpiGear('copq')}</div><div class="metric kpi-value-copq">${dashPct(a.copqPct)}</div><div class="kpi-cost-sub">${dashMoney(a.scrapCost)} cost of poor quality</div>${kpiDeltaMarkup(a.copqPct,prev.copqPct,'points',false,priorLabel)}</div>
 </div>
