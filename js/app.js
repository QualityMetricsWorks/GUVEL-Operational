...
function dashTab(t){dashboardState.tab=t;const dash=document.getElementById('dash');if(!dash)return;if(t!=='General'){dash.innerHTML=`<div class="panel dashboard-coming"><div class="eyebrow">PHASE 1.9</div><h2>${escapeHtml(t)} Dashboard</h2><p>This section is reserved for the next Dashboard subphase. General Dashboard is the current implementation.</p><span class="dashboard-phase-badge">Implementation deferred</span></div>`;return;}dash.innerHTML=`<div id="dashboardGeneral"></div>`;renderDashboardGeneral();}
...
function dashboardFiltered(rangeOverride=null){const f=dashboardState.filters;const range=rangeOverride||dashboardRange();const prod=dashboardState.production.filter(r=>(!range.from||r.production_date>=range.from)&&(!range.to||r.production_date<=range.to)&&(!f.customer||r.customer_id===f.customer)&&(!f.part||r.part_number_id===f.part)&&(!f.shift||r.shift_id===f.shift)&&(!f.machine||r.machine_id===f.machine));const ids=new Set(prod.map(r=>r.id));return {prod,scrap:dashboardState.scrap.filter(r=>ids.has(r.production_capture_id)),downtime:dashboardState.downtime.filter(r=>ids.has(r.production_capture_id))};}
function dashboardAggregate(rangeOverride=null){const d=dashboardFiltered(rangeOverride),scrapByCapture=new Map(),downByCapture=new Map(),partStats=new Map();for(const r of d.scrap)scrapByCapture.set(r.production_capture_id,(scrapByCapture.get(r.production_capture_id)||0)+Number(r.quantity||0));for(const r of d.downtime){const a=downByCapture.get(r.production_capture_id)||{planned:0,unplanned:0,unknown:0,total:0};const m=Number(r.minutes||0);a.total+=m;const type=String(r.event_type||'').toLowerCase();if(type==='planned')a.planned+=m;else if(type==='unplanned')a.unplanned+=m;else a.unknown+=m;downByCapture.set(r.production_capture_id,a);}
 let production=0,scrap=0,scrapCost=0,planned=0,operating=0,perfNumerator=0,validOee=0,invalidOee=0,unknownEvents=0,plannedDowntime=0,unplannedDowntime=0;
 const daily=new Map();
 for(const r of d.prod){const q=Number(r.production_quantity||0),sq=scrapByCapture.get(r.id)||0;production+=q;scrap+=sq;scrapCost+=sq*Number(r.part_numbers?.scrap_cost||0);const ps=partStats.get(r.part_number_id)||{production:0,scrap:0};ps.production+=q;ps.scrap+=sq;partStats.set(r.part_number_id,ps);const dt=downByCapture.get(r.id)||{planned:0,unplanned:0,unknown:0,total:0};plannedDowntime+=dt.planned;unplannedDowntime+=dt.unplanned;unknownEvents+=dt.unknown>0?1:0;const sh=r.shifts,shiftSec=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,plannedSec=shiftSec==null?null:Math.max(0,shiftSec-excluded),cycle=Number(r.operations?.ideal_cycle_time_seconds||0);if(plannedSec!=null&&cycle>0&&dt.unknown===0){const opSec=Math.max(0,plannedSec-dt.unplanned*60);planned+=plannedSec;operating+=opSec;perfNumerator+=cycle*q;validOee++;}else invalidOee++;const key=r.production_date||'No Date';const day=daily.get(key)||{production:0,scrap:0};day.production+=q;day.scrap+=sq;daily.set(key,day);}
 const good=production-scrap,yieldRatio=production>0?good/production:null,ppm=production>0?scrap/production*1e6:null,availability=planned>0?operating/planned:null,performance=operating>0?perfNumerator/operating:null,quality=production>0?good/production:null,oee=availability!=null&&performance!=null&&quality!=null?availability*performance*quality:null;
 const partRates=[...partStats.values()].filter(x=>x.production>0).map(x=>x.scrap/x.production);const scrapPct=partRates.length?partRates.reduce((a,b)=>a+b,0)/partRates.length:null;
 return {d,production,scrap,scrapCost,good,yieldRatio,scrapPct,ppm,planned,operating,perfNumerator,availability,performance,quality,oee,validOee,invalidOee,unknownEvents,plannedDowntime,unplannedDowntime,daily,partStats};}
...
function kpiDeltaMarkup(current,previous,mode,higherIsBetter,label){const d=dashDelta(current,previous,mode);const cls=dashDeltaClass(d,higherIsBetter);const arrow=d==null?'—':d>0?'↑':d<0?'↓':'→';const text=d==null?'No prior data':`${arrow} ${mode==='points'?dashSignedPct(d):dashSignedPct(d)}`;return `<div class="kpi-compare ${cls}"><span>${text}</span><small>vs ${escapeHtml(label)}</small></div>`;}
...
function renderDashboardGeneral(){const box=document.getElementById('dashboardGeneral');if(!box)return;const cmp=dashboardComparison(),a=cmp.current,prev=cmp.previous,p=cmp.period;if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p></div>';dashboardStatus('No Data in the selected filter scope.');return;}
 ...
 const priorLabel=p.period.replace('This ','Previous ').replace('Today','Previous Day');
 const good=a.good;
 ...
 <div class="card kpi-card kpi-oee"><div class="kpi-top"><div class="label">OEE</div><span class="kpi-info">Availability × Performance × Quality</span></div><div class="metric">${dashPct(a.oee)}</div>${kpiDeltaMarkup(a.oee,prev.oee,'points',true,priorLabel)}</div>
 <div class="card kpi-card kpi-production"><div class="kpi-top"><div class="label">Production</div><span class="kpi-info">Pieces recorded</span></div><div class="metric">${dashNum(a.production)}</div>${kpiDeltaMarkup(a.production,prev.production,'relative',true,priorLabel)}</div>
 <div class="card kpi-card kpi-scrap"><div class="kpi-top"><div class="label">Scrap</div><span class="kpi-info">Average by Part Number</span></div><div class="metric">${dashPct(a.scrapPct)}</div>${kpiDeltaMarkup(a.scrapPct,prev.scrapPct,'points',false,priorLabel)}</div>
 <div class="card kpi-card kpi-ppm"><div class="kpi-top"><div class="label">PPMs</div><span class="kpi-info">Scrap / production</span></div><div class="metric">${a.ppm==null?'N/A':Math.round(a.ppm).toLocaleString()}</div>${kpiDeltaMarkup(a.ppm,prev.ppm,'relative',false,priorLabel)}</div>
 <div class="card kpi-card kpi-yield"><div class="kpi-top"><div class="label">Yield</div><span class="kpi-info">Good / production</span></div><div class="metric">${dashPct(a.yieldRatio)}</div>${kpiDeltaMarkup(a.yieldRatio,prev.yieldRatio,'points',true,priorLabel)}</div>
 <div class="card kpi-card kpi-copq"><div class="kpi-top"><div class="label">COPQ</div><span class="kpi-info">Direct scrap cost</span></div><div class="metric">${dashMoney(a.scrapCost)}</div>${kpiDeltaMarkup(a.scrapCost,prev.scrapCost,'relative',false,priorLabel)}</div>
 </div>
 <div class="grid dashboard-secondary-kpis"><div class="card"><div class="label">Availability</div><div class="metric">${dashPct(a.availability)}</div></div><div class="card"><div class="label">Performance</div><div class="metric">${dashPct(a.performance)}</div></div><div class="card"><div class="label">Good Pieces</div><div class="metric">${dashNum(good)}</div></div><div class="card"><div class="label">Unplanned Downtime</div><div class="metric">${dashNum(a.unplannedDowntime)} min</div></div></div>
 <div class="dashboard-chart-grid">
 <div class="panel chart-panel chart-wide"><div class="chart-head"><div><h2>OEE Trend</h2><p>Daily OEE across the selected period.</p></div><span class="chart-chip">${escapeHtml(p.label)}</span></div><div class="chart-wrap"><canvas id="chartOee"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Production</h2><p>Daily pieces produced.</p></div><span class="chart-chip">Pieces</span></div><div class="chart-wrap"><canvas id="chartProduction"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Scrap %</h2><p>Average scrap rate by part number.</p></div><span class="chart-chip">Lower is better</span></div><div class="chart-wrap"><canvas id="chartScrap"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>PPM</h2><p>Scrap pieces per million produced.</p></div><span class="chart-chip">PPM</span></div><div class="chart-wrap"><canvas id="chartPpm"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Yield</h2><p>Good pieces as a percentage of production.</p></div><span class="chart-chip">Higher is better</span></div><div class="chart-wrap"><canvas id="chartYield"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Quality Loss</h2><p>Scrap quantity by day.</p></div><span class="chart-chip">Pieces</span></div><div class="chart-wrap"><canvas id="chartQualityLoss"></canvas></div></div>
 </div>
 <div class="dashboard-bottom-grid"><div class="panel"><div class="chart-head"><div><h2>Top Part Numbers by Scrap</h2><p>Where the largest quality loss is concentrated.</p></div></div><div id="topParts"></div></div><div class="panel"><div class="chart-head"><div><h2>Operational Snapshot</h2><p>Current period compared with the previous period.</p></div></div><div class="comparison-list">${[['Production',a.production,prev.production,'relative',true],['Scrap %',a.scrapPct,prev.scrapPct,'points',false],['PPM',a.ppm,prev.ppm,'relative',false],['Yield',a.yieldRatio,prev.yieldRatio,'points',true],['COPQ',a.scrapCost,prev.scrapCost,'relative',false]].map(x=>`<div class="comparison-row"><span>${x[0]}</span><strong>${x[0]==='Production'?dashNum(x[1]):x[0]==='COPQ'?dashMoney(x[1]):x[0]==='PPM'?(x[1]==null?'N/A':Math.round(x[1]).toLocaleString()):dashPct(x[1])}</strong><em class="${dashDeltaClass(dashDelta(x[1],x[2],x[3]),x[4])}">${dashSignedPct(dashDelta(x[1],x[2],x[3]))}</em></div>`).join('')}</div></div></div>`;
 dashboardStatus(`${a.d.prod.length.toLocaleString()} production captures · ${a.scrap.length.toLocaleString()} scrap events · ${a.downtime.length.toLocaleString()} downtime events.`);updateDashboardPeriodHint(p);renderDashboardCharts(cmp);renderTopParts(a);}
...
function chartBase(scales={}){return {responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{mode:'index',intersect:false}},interaction:{mode:'index',intersect:false},scales:{x:{grid:{display:false},ticks:{maxRotation:0}},y:{beginAtZero:true,...scales}}};}
function renderDashboardCharts(cmp){if(!window.Chart)return;const labels=niceLabels(cmp.current.daily);const prod=labels.map(x=>cmp.current.daily.get(x)?.production||0),scrap=labels.map(x=>cmp.current.daily.get(x)?.scrap||0),oee=labels.map(day=>{const sub=dashboardAggregate({from:day,to:day});return sub.oee==null?null:sub.oee*100;}),yieldVals=labels.map(day=>{const sub=dashboardAggregate({from:day,to:day});return sub.yieldRatio==null?null:sub.yieldRatio*100;}),ppmVals=labels.map(day=>{const sub=dashboardAggregate({from:day,to:day});return sub.ppm==null?null:sub.ppm;}),scrapPctVals=labels.map(day=>{const sub=dashboardAggregate({from:day,to:day});return sub.scrapPct==null?null:sub.scrapPct*100;});
 chartCreate('oee',document.getElementById('chartOee'),{type:'line',data:{labels,datasets:[{data:oee,borderWidth:3,pointRadius:3,tension:.35,fill:true,backgroundColor:'rgba(12,192,223,.12)',borderColor:'#0cc0df',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{min:0,max:100,ticks:{callback:v=>v+'%'}}})});
 chartCreate('production',document.getElementById('chartProduction'),{type:'bar',data:{labels,datasets:[{data:prod,borderRadius:3,backgroundColor:'rgba(12,192,223,.72)',borderColor:'#0cc0df',borderWidth:1}]},options:chartBase()});
 chartCreate('scrap',document.getElementById('chartScrap'),{type:'line',data:{labels,datasets:[{data:scrapPctVals,borderWidth:3,pointRadius:3,tension:.35,fill:true,backgroundColor:'rgba(255,49,49,.10)',borderColor:'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{beginAtZero:true,ticks:{callback:v=>v+'%'}}})});
 chartCreate('ppm',document.getElementById('chartPpm'),{type:'line',data:{labels,datasets:[{data:ppmVals,borderWidth:3,pointRadius:3,tension:.35,fill:true,backgroundColor:'rgba(255,49,49,.08)',borderColor:'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase()});
 chartCreate('yield',document.getElementById('chartYield'),{type:'line',data:{labels,datasets:[{data:yieldVals,borderWidth:3,pointRadius:3,tension:.35,fill:true,backgroundColor:'rgba(12,192,223,.12)',borderColor:'#0cc0df',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{min:80,max:100,ticks:{callback:v=>v+'%'}}})});
 chartCreate('qualityLoss',document.getElementById('chartQualityLoss'),{type:'bar',data:{labels,datasets:[{data:scrap,borderRadius:3,backgroundColor:'rgba(255,49,49,.72)',borderColor:'#ff3131',borderWidth:1}]},options:chartBase()});}
...
function bindDashboard(){const c=document.getElementById('dashCustomer'),p=document.getElementById('dashPart'),s=document.getElementById('dashShift'),m=document.getElementById('dashMachine'),f=document.getElementById('dashFrom'),to=document.getElementById('dashTo'),period=document.getElementById('dashPeriod');document.querySelectorAll('[data-dashboard-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-dashboard-tab]').forEach(x=>x.classList.toggle('active',x===b));dashTab(b.dataset.dashboardTab);});const apply=()=>{dashboardState.filters={...dashboardState.filters,from:f.value,to:to.value};renderDashboardGeneral();};[f,to,p,s,m].forEach(x=>x.onchange=apply);period.onchange=()=>{const r=resolvePeriod(period.value);dashboardState.filters={...dashboardState.filters,period:period.value,from:r.from,to:r.to};f.value=r.from;to.value=r.to;renderDashboardGeneral();};c.onchange=()=>{dashboardState.filters.customer=c.value;dashboardState.filters.part='';populateDashboardFilters();renderDashboardGeneral();};document.getElementById('dashboardClear').onclick=()=>{dashboardState.filters={period:'This Month',from:resolvePeriod('This Month').from,to:resolvePeriod('This Month').to,customer:'',part:'',shift:'',machine:''};populateDashboardFilters();renderDashboardGeneral();};document.getElementById('dashboardRefresh').onclick=loadDashboardData;const initial=resolvePeriod(dashboardState.filters.period||'This Month');if(!dashboardState.filters.from)dashboardState.filters.from=initial.from;if(!dashboardState.filters.to)dashboardState.filters.to=initial.to;populateDashboardFilters();loadDashboardData();}
