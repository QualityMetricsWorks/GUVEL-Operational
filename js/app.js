...
function dashboardAggregate(rangeOverride=null){const d=dashboardFiltered(rangeOverride),scrapByCapture=new Map(),downByCapture=new Map(),partStats=new Map();for(const r of d.scrap)scrapByCapture.set(r.production_capture_id,(scrapByCapture.get(r.production_capture_id)||0)+Number(r.quantity||0));for(const r of d.downtime){const a=downByCapture.get(r.production_capture_id)||{planned:0,unplanned:0,unknown:0,total:0};const m=Number(r.minutes||0);a.total+=m;const type=String(r.event_type||'').toLowerCase();if(type==='planned')a.planned+=m;else if(type==='unplanned')a.unplanned+=m;else a.unknown+=m;downByCapture.set(r.production_capture_id,a);}
 ...
 const totalProducedCost=goodCost+poorCost;const copqPct=totalProducedCost>0?poorCost/totalProducedCost:null;
 return {d,production,scrap,scrapCost,good,goodCost,poorCost,totalProducedCost,copqPct,yieldRatio,scrapPct,ppm,planned,operating,perfNumerator,availability,performance,quality,oee,validOee,invalidOee,unknownEvents,plannedDowntime,unplannedDowntime,daily,partStats};}
...
function kpiDeltaMarkup(current,previous,mode,higherIsBetter,label){const d=dashDelta(current,previous,mode);const cls=dashDeltaClass(d,higherIsBetter);const arrow=d==null?'—':d>0?'↑':d<0?'↓':'→';let text='No prior data';if(d!=null){const sign=d>0?'+':d<0?'−':'';text=mode==='points'?`${arrow} ${sign}${Math.abs(d*100).toLocaleString(undefined,{minimumFractionDigits:1,maximumFractionDigits:2})} pp`:`${arrow} ${sign}${Math.abs(d*100).toLocaleString(undefined,{minimumFractionDigits:1,maximumFractionDigits:1})}%`;}return `<div class="kpi-compare ${cls}"><span>${text}</span><small>vs ${escapeHtml(label)}</small></div>`;}
...
function renderDashboardGeneral(){const box=document.getElementById('dashboardGeneral');if(!box)return;const cmp=dashboardComparison(),a=cmp.current,prev=cmp.previous,p=cmp.period;if(!a.d.prod.length){box.innerHTML='<div class="panel dashboard-empty"><div class="eyebrow">GENERAL</div><h2>No Data</h2><p>No production captures match the selected Dashboard filters.</p></div>';dashboardStatus('No Data in the selected filter scope.');return;}
 ...
 <div class="card kpi-card kpi-copq"><div class="kpi-top"><div class="label">COPQ</div><span class="kpi-info">Poor Quality Cost / Total Cost</span></div><div class="metric">${dashPct(a.copqPct)}</div><div class="kpi-cost-sub">${dashMoney(a.scrapCost)} cost of poor quality</div>${kpiDeltaMarkup(a.copqPct,prev.copqPct,'points',false,priorLabel)}</div>
 </div>
 <div class="dashboard-chart-grid dashboard-chart-grid-six">
 <div class="panel chart-panel"><div class="chart-head"><div><h2>OEE</h2><p>Daily OEE.</p></div><span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartOee"></canvas></div></div>
 ...
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Scrap %</h2><p>Average scrap rate by part number.</p></div><span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartScrap"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>PPMs</h2><p>Scrap pieces per million produced.</p></div><span class="chart-chip">PPM</span></div><div class="chart-wrap"><canvas id="chartPpm"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>Yield</h2><p>Good pieces / production.</p></div><span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartYield"></canvas></div></div>
 <div class="panel chart-panel"><div class="chart-head"><div><h2>COPQ %</h2><p>Poor quality cost / total produced cost.</p></div><span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="chartCopq"></canvas></div></div>
 </div>`;
...
function renderDashboardCharts(cmp){if(!window.Chart){dashboardStatus('Chart library unavailable.','error');return;}const labels=niceLabels(cmp.current.daily);const daily=labels.map(day=>dashboardDailyMetric(day));const prod=daily.map(x=>x.production),oee=daily.map(x=>x.oee==null?null:x.oee*100),yieldVals=daily.map(x=>x.yieldRatio==null?null:x.yieldRatio*100),ppmVals=daily.map(x=>x.ppm==null?null:x.ppm),scrapPctVals=daily.map(x=>x.scrapPct==null?null:x.scrapPct*100),copqVals=daily.map(x=>x.copqPct==null?null:x.copqPct*100);
 const tip=(unit='')=>({callbacks:{label:c=>`${c.dataset.label||''}: ${c.parsed.y==null?'N/A':Number(c.parsed.y).toLocaleString(undefined,{maximumFractionDigits:2})}${unit}`}});
 chartCreate('oee',document.getElementById('chartOee'),{type:'line',data:{labels,datasets:[{label:'OEE',data:oee,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(12,192,223,.12)',borderColor:'#0cc0df',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{min:0,max:100,ticks:{callback:v=>v+'%'}}})});
 chartCreate('production',document.getElementById('chartProduction'),{type:'bar',data:{labels,datasets:[{label:'Production',data:prod,borderRadius:3,backgroundColor:'rgba(12,192,223,.72)',borderColor:'#0cc0df',borderWidth:1}]},options:chartBase()});
 chartCreate('scrap',document.getElementById('chartScrap'),{type:'line',data:{labels,datasets:[{label:'Scrap %',data:scrapPctVals,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(255,49,49,.10)',borderColor:'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{beginAtZero:true,ticks:{callback:v=>v+'%'}}})});
 chartCreate('ppm',document.getElementById('chartPpm'),{type:'line',data:{labels,datasets:[{label:'PPM',data:ppmVals,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(255,49,49,.08)',borderColor:'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase()});
 chartCreate('yield',document.getElementById('chartYield'),{type:'line',data:{labels,datasets:[{label:'Yield',data:yieldVals,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(12,192,223,.12)',borderColor:'#0cc0df',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{min:80,max:100,ticks:{callback:v=>v+'%'}}})});
 chartCreate('copq',document.getElementById('chartCopq'),{type:'line',data:{labels,datasets:[{label:'COPQ %',data:copqVals,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(255,49,49,.10)',borderColor:'#ff3131',pointBackgroundColor:'#fff',pointBorderWidth:2}]},options:chartBase({y:{beginAtZero:true,ticks:{callback:v=>v+'%'}}})});}
