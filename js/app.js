...
const PROD_CHART_META={
  ...
  prod_components:{label:'OEE Components',unit:'%'},
  ...
  prod_downtime:{label:'Downtime by Machine',unit:'minutes'}
};
...
function productionDashboardAggregate(rangeOverride=null){
  const d=dashboardFiltered(rangeOverride),scrapByCapture=new Map(),downByCapture=new Map();
  ...
  const groups=new Map(),daily=new Map(),machine=new Map(),shift=new Map();
  let production=0,scrap=0,good=0,planned=0,operating=0,perfNumerator=0,valid=0,invalid=0,plannedDowntime=0,unplannedDowntime=0;
  const add=(map,key,seed)=>{let x=map.get(key);if(!x){x={...seed};map.set(key,x);}return x;};
  for(const r of d.prod){
    const q=Number(r.production_quantity||0),sq=scrapByCapture.get(r.id)||0,gd=Math.max(0,q-sq),dt=downByCapture.get(r.id)||{planned:0,unplanned:0,unknown:0,total:0};
    const sh=r.shifts,shiftSec=shiftDurationSeconds(sh),excluded=Number(sh?.excluded_planned_minutes||0)*60,plannedSec=shiftSec==null?null:Math.max(0,shiftSec-excluded),cycle=Number(r.operations?.ideal_cycle_time_seconds||0);
    production+=q;scrap+=sq;good+=gd;plannedDowntime+=dt.planned;unplannedDowntime+=dt.unplanned;
    const dayKey=String(r.production_date||'').slice(0,10)||'No Date';
    const mKey=r.machine_id||'unknown-machine',m=add(machine,mKey,{production:0,scrap:0,good:0,planned:0,operating:0,perfNumerator:0,unplanned:0});m.production+=q;m.scrap+=sq;m.good+=gd;m.unplanned+=dt.unplanned;
    const shKey=r.shift_id||'unknown-shift',sx=add(shift,shKey,{production:0,scrap:0,good:0,planned:0,operating:0,perfNumerator:0});sx.production+=q;sx.scrap+=sq;sx.good+=gd;
    if(plannedSec!=null){
      const gKey=`${dayKey}|${shKey}|${mKey}`;const g=add(groups,gKey,{planned:0,operating:0,perfNumerator:0,production:0,scrap:0,good:0,unknown:0,idealWeighted:0,actualSeconds:0});
      if(g.planned===0)g.planned+=plannedSec;
      const opSec=Math.max(0,plannedSec-dt.unplanned*60);g.operating+=opSec;g.production+=q;g.scrap+=sq;g.good+=gd;g.perfNumerator+=cycle*q;g.idealWeighted+=cycle*q;g.actualSeconds+=opSec;g.unknown+=dt.unknown;
      const day=add(daily,dayKey,{production:0,scrap:0,good:0,planned:0,operating:0,perfNumerator:0,unknown:0});day.production+=q;day.scrap+=sq;day.good+=gd;day.planned+=plannedSec;day.operating+=opSec;day.perfNumerator+=cycle*q;day.unknown+=dt.unknown;
      m.planned+=plannedSec;m.operating+=opSec;m.perfNumerator+=cycle*q;
      sx.planned+=plannedSec;sx.operating+=opSec;sx.perfNumerator+=cycle*q;
    }
    if(plannedSec!=null&&cycle>0&&dt.unknown===0)valid++;else invalid++;
  }
  planned=[...groups.values()].reduce((sum,x)=>sum+x.planned,0);
  operating=[...groups.values()].reduce((sum,x)=>sum+x.operating,0);
  perfNumerator=[...groups.values()].reduce((sum,x)=>sum+x.perfNumerator,0);
  const calc=x=>{const availability=x.planned>0?x.operating/x.planned:null,performance=x.operating>0?x.perfNumerator/x.operating:null,quality=x.production>0?x.good/x.production:null,oee=availability!=null&&performance!=null&&quality!=null?availability*performance*quality:null,actualCycle=x.production>0?x.operating/x.production:null;return {...x,availability,performance,quality,oee,actualCycle};};
  const dailyCalc=new Map([...daily.entries()].map(([k,v])=>[k,calc(v)]));
  const machineCalc=new Map([...machine.entries()].map(([k,v])=>[k,calc(v)]));
  const shiftCalc=new Map([...shift.entries()].map(([k,v])=>[k,calc(v)]));
  const validGroups=[...groups.values()].filter(x=>x.planned>0&&x.operating>0&&x.production>0&&x.unknown===0);
  const all={production,scrap,good,planned,operating,perfNumerator,plannedDowntime,unplannedDowntime,...calc({production,scrap,good,planned,operating,perfNumerator}),valid,invalid,daily:dailyCalc,machine:machineCalc,shift:shiftCalc};
  return {d,...all};
}
...
function renderProductionDashboard(){
  ...
  <div class="dashboard-chart-grid production-chart-grid">
    ...
    <div class="panel chart-panel"><div class="chart-head"><div><h2>OEE Components</h2><p>Availability, Performance and Quality.</p></div>${chartGear('prod_components')}<span class="chart-chip">%</span></div><div class="chart-wrap"><canvas id="prodChartComponents"></canvas></div></div>
    ...
    <div class="panel chart-panel chart-wide"><div class="chart-head"><div><h2>Downtime by Machine</h2><p>Planned and unplanned downtime minutes.</p></div>${chartGear('prod_downtime')}<span class="chart-chip">Minutes</span></div><div class="chart-wrap"><canvas id="prodChartDowntime"></canvas></div></div>
  </div>`;
...
function renderProductionCharts(cmp){if(!window.Chart){dashboardStatus('Chart library unavailable.','error');return;}const labels=[...cmp.current.daily.keys()].sort(),days=labels.map(k=>cmp.current.daily.get(k)),oee=days.map(x=>x.oee==null?null:x.oee*100),prod=days.map(x=>x.production),good=days.map(x=>x.good),scrap=days.map(x=>x.scrap),av=days.map(x=>x.availability==null?null:x.availability*100),perf=days.map(x=>x.performance==null?null:x.performance*100),qual=days.map(x=>x.quality==null?null:x.quality*100);const yPct=k=>{const c=dashboardPrefs().charts?.[k]||{},o={beginAtZero:true,ticks:{callback:v=>v+'%'}};if(c.min!=null)o.min=c.min*100;if(c.max!=null)o.max=c.max*100;return o;};const refs=k=>chartReferenceDatasets(k,labels);const line=(key,id,data,extra={})=>chartCreate(key,document.getElementById(id),{type:'line',data:{labels,datasets:[{label:DASH_CHART_META[key].label,data,borderWidth:3,pointRadius:4,pointHoverRadius:7,tension:.3,fill:true,backgroundColor:'rgba(12,192,223,.10)',borderColor:'#0cc0df',pointBackgroundColor:'#fff',pointBorderWidth:2},...refs(key)]},options:chartBase({y:extra.pct?yPct(key):{beginAtZero:true,...extra}})});
  ...
  chartCreate('prod_components',document.getElementById('prodChartComponents'),{type:'line',data:{labels,datasets:[{label:'Availability',data:av,borderWidth:2.5,pointRadius:3,tension:.25,borderColor:'#0cc0df'},{label:'Performance',data:perf,borderWidth:2.5,pointRadius:3,tension:.25,borderColor:'#143980'},{label:'Quality',data:qual,borderWidth:2.5,pointRadius:3,tension:.25,borderColor:'#006732'},...refs('prod_components')]},options:chartBase({y:yPct('prod_components')})});
  ...
  const downMap=new Map();for(const r of cmp.current.d.downtime){const id=cmp.current.d.prod.find(p=>p.id===r.production_capture_id)?.machine_id||'unknown-machine';const x=downMap.get(id)||{planned:0,unplanned:0};const mins=Number(r.minutes||0);const type=String(r.event_type||'').toLowerCase();if(type==='planned')x.planned+=mins;else if(type==='unplanned')x.unplanned+=mins;downMap.set(id,x);}const downEntries=[...downMap.entries()].sort((a,b)=>(b[1].planned+b[1].unplanned)-(a[1].planned+a[1].unplanned));const downLabels=downEntries.map(([id])=>productionMetricLabel(id));chartCreate('prod_downtime',document.getElementById('prodChartDowntime'),{type:'bar',data:{labels:downLabels,datasets:[{label:'Planned',data:downEntries.map(([,v])=>v.planned),backgroundColor:'rgba(20,57,128,.45)',borderColor:'#143980',borderWidth:1},{label:'Unplanned',data:downEntries.map(([,v])=>v.unplanned),backgroundColor:'rgba(255,49,49,.68)',borderColor:'#ff3131',borderWidth:1},...chartReferenceDatasets('prod_downtime',downLabels)]},options:chartBase({y:{beginAtZero:true}})});
  bindDashboardConfig();
