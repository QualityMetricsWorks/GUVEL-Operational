...
function bindDashboardConfig(){document.querySelectorAll('[data-kpi-settings]').forEach(b=>b.onclick=()=>openKpiSettings(b.dataset.kpiSettings));document.querySelectorAll('[data-chart-settings]').forEach(b=>b.onclick=()=>openChartSettings(b.dataset.chartSettings));const e=dashboardConfigOverlay();e.onclick=ev=>{if(ev.target===e||ev.target.closest('[data-config-close]'))return closeDashboardConfig();const ks=ev.target.closest('[data-kpi-save]');if(ks)return saveKpiSettings(ks.dataset.kpiSave);const cs=ev.target.closest('[data-chart-save]');if(cs)return saveChartSettings(cs.dataset.chartSave);const rs=ev.target.closest('[data-config-reset]');if(rs){const title=e.querySelector('h2')?.textContent||'';const key=Object.keys(DASH_KPI_META).find(k=>DASH_KPI_META[k].label===title)||Object.keys(DASH_CHART_META).find(k=>DASH_CHART_META[k].label===title);if(!key)return;return resetDashboardConfig(DASH_KPI_META[key]?'kpi':'chart',key);}};}
